...
size_t i; //Indice array
    ...
    if((risultatiPtr = fopen("RISULTATI.txt", "w")) == NULL){
        puts("Impossibile creare file di testo con risultati");
    }
        ...
        *operandPtr = *instructionRegisterPtr % 100; //Ricava l'operando
        ...
        if(*operationCodePtr == 10){ //Istruzione READ
            do{
                ...
                fprintf(risultatiPtr, "Risultato dell'elaborazione: %+5lf\n", memoryPtr[ *operandPtr ]);
        }
            ...
            fprintf(risultatiPtr, "%s", "*** Simpletron execution terminated ***");
            return 1;
        }
        else if(*operationCodePtr == 60){ //Istruzione NEWLINE
                printf("%s\n", "");
                fprintf(risultatiPtr, "%s\n", "");
                ...
                while(i < buffer){
                    if(i % 2 == 0){//Se indice pari, la lettera va nella parte destra della cella di memoria
                        ...
                        *operandPtr += 1;
                        ...
                        fprintf(risultatiPtr, "%c", stringa[i] = floor(( memoryPtr[ *operandPtr ] / 100 )));
        ...
        if( debugMode == 1 ){
            ...
            debugDump( memoryPtr, accumulatorPtr, instructionCounterPtr, instructionRegisterPtr, operationCodePtr, operandPtr, debugMode );
        }
        /*DEBUG MODE*/
